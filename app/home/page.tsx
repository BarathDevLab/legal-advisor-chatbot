...
interface Message {
  ...
  isFollowUp?: boolean;
  suggestedQuestions?: string[]; // Store random questions for each message
}
...
export default function Home() {
  ...
  // Pool of suggested questions for employment law
  const allSuggestedQuestions = [
    "Who do I contact if my employer refuses to give me a written contract?",
    "How can I file a complaint against an abusive employer?",
    "What are my rights if I am terminated without notice?",
    "Is it legal to terminate me during maternity leave?",
    "Can I challenge an unfair dismissal at work?",
    "What rights do contract workers have in India?",
    "Can an employer cut my pay without my consent?",
    "What should I do if my salary is delayed?",
    "How can I report workplace harassment or discrimination?",
    "Can I be fired for refusing to work extra hours?",
  ];

  // Category-specific questions for Watson follow-up
  ...
  // Function to get random questions - or category questions if Watson is asking for category
  const getRandomQuestions = (count: number = 3, watsonResponse?: string) => {
    // Check if Watson is asking for legal category specification
    if (
      watsonResponse
        ?.toLowerCase()
        .includes("specify the exact section of legal rights")
    ) {
      return Object.values(categoryQuestions);
    }
  ...
  // Function to get the appropriate label for suggested questions
  const getSuggestedQuestionsLabel = (watsonResponse?: string) => {
    if (
      watsonResponse
        ?.toLowerCase()
        .includes("specify the exact section of legal rights")
    ) {
      return "📋 Currently available section:";
    }
  ...
  useEffect(() => {
    initSession();
    // Generate welcome questions once on mount
    setWelcomeQuestions(getRandomQuestions(4));
  }, []);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend returns watson_stage (singular), not watson_responses (array)
      const watsonStage = data.watson_stage;
      const isGoodbyeFlag = watsonStage?.is_goodbye || false;
      console.log("isGoodbye:", isGoodbyeFlag, "Watson stage:", watsonStage);
      ...
      // Backend only sends simplified_answer (not detailed_answer)
      ...
      const assistantMessage: Message = {
        ...
        suggestedQuestions: getRandomQuestions(3, watsonStage?.response), // Pass Watson response to check for category question
      };

      setMessages((prev) => [...prev, assistantMessage]);

      // Update conversation state directly from backend response
              ...
              <div className="mt-8 space-y-4">
                <div className="text-center">
                  <p className="text-sm text-muted-foreground mb-4">
                    💡 Try asking about employment law:
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {welcomeQuestions.map((question, idx) => (
                    <button
                      key={idx}
            ...
            <div className="flex justify-center">
              <button
                onClick={() => {
                  ...
                  initSession();
                  // Generate new random questions for the new session
                  setWelcomeQuestions(getRandomQuestions(4));
                }}
